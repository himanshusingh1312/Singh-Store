...
const Tshirts = () => {
  ...
  return (
   ...
   <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-3 lg:grid-cols-5  sm:gap-4  gap-3 justify-center  mx-3 ls:mx-0">
     {order.map((item) => (
       <Link key={item.slug} href={`/products/${item._id}`}>
         <div className="p-2  ls:p-4  w-full  ">
           <div className="relative overflow-hidden rounded-lg shadow-lg transition duration-300 ease-in-out transform hover:scale-95">
             <img
               alt="ecommerce"
               className="object-cover object-center w-full h-28 xs:h-32 sm:h-36 md:h-40 lg:h-56 "
               src={item.img}
             />
           </div>
